...
from logistic_sgd import LogisticRegression, load_data, get_author_number, alphabet
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=100, batch_size=10, n_hidden=26):
    """
   ...
   """
    ...
    validation_set_x, validation_set_y = datasets[2]

    # Empty the file
    file = open('weightsValues.txt', 'a')
    file.write('[train_set_y]\n')
    file.write('%s' %train_set_y)
    file.write('\n\n\n')
    ...
    x = T.matrix('x')   # the data is presented as 1D vector of
                        # [int] data
    ...
    while (epoch < n_epochs):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            # # # Empty the file
            # file.write('%s\n' %train_set_x[minibatch_index * batch_size: (minibatch_index + 1) * batch_size].eval())
